...
const CarCard = ({ car }) => {
    ...
    return (
        <div onClick={ ()=> {navigate(`/car-details/${car.id}`); scrollTo(0,0)}} className='group rounded-xl overflow-hidden shadow-lg hover:-translate-y-1
        transition-all duration-500 cursor-pointer'>
            <div className='relative h-48 overflow-hidden'>
                {/* img info */}
                <img src={car.image} alt="Car Image" className='w-full h-full object-cover
                transition-transform duration-500 group-hover:scale-105'/>
                {car.available && <p className='absolute top-4 left-4 bg-primary/90
