...
const CarDetails = () => {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post('/api/bookings/create',{car ,pickupDate,returnDate})
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12'>
              ...
              <ul className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                {
                  ["360 Camera", "Bluetooth", "GPS", "Heated Seats", "Rear View Mirror"].map((feature)=>(
                    <li key={feature} className='flex items-center text-gray-500'>
                      <img src={assets.check_icon} alt="check" className='h4 mr-2'/>
                      {feature}
                    </li>
                  ))
                }
        ...
        onSubmit={handleSubmit} className='shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <input value={pickupDate} onChange={(e)=>setPickupDate(e.target.value)} type="date" required id="pickup-date" min={new Date().toISOString().split('T')[0]} className='border border-borderColor px-3 py-2 rounded-lg'/>
            </div>
            {/* Return-date */}
            <div className='flex flex-col gap-2'>
              <label htmlFor="return-date">Return Date</label>
              <input value={returnDate} onChange={(e)=>setReturnDate(e.target.value)} type="date" required id="return-date" min={new Date().toISOString().split('T')[0]} className='border border-borderColor px-3 py-2 rounded-lg'/>
            </div>

            <button className='bg-primary w-full hover:bg-primary-dull transition-all py-3 font-medium text-white rounded-xl cursor-pointer '>Book Now</button>
