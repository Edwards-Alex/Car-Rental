...
const MyBookings = () => {
  ...
  const fetchMyBookings = async () => {
    try {
      const { data } = await axios.get(`/api/bookings/user?timestamp=${Date.now()}`)
      if (data.success) {
  ...
  //fetchBookings when page loading
  useEffect(() => {
          ...
          <MOTION.div
            ...
            <div className='md:col-span-2'>
              <div className='flex items-center gap-2'>
                ...
                <p className={`px-3 py-1 text-xs rounded-full ${booking.status.toLowerCase() === 'confirmed' ? 'bg-green-400/15 text-green-600' : 'bg-red-400/15 text-red-600'}`}>{booking.status}</p>
              </div>
